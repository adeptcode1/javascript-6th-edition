...
var countdown;
var ticket = {
    //date: "", removed date property per instruction on page 499 of text.
...
function displayCalendar(whichMonth) {
    ...
    if (month === 0 || month === 2 || month === 4 ||
        ...
        month === 11) { // January, March, May, July, August, October, December
        daysInMonth = 31;
...
function selectDate(event) {
    if (event === undefined) { // get window.event if event argument missing (in IE)
        event = window.event;
    }
    ...
    /*  countdownSection is a child of ticket section element. Display of countdown timer will not display
    without display of ticket section due to inheritance. Display attribute of ticket section 
    set to block to allow display of countdown timer for testing prior to registerName function creation.
    Comment out attribute change after registerName is created.
    */
...
function createEventListeners() {
    ...
    if (prevLink.addEventListener) {
        prevLink.addEventListener("click", prevMonth, false);
        nextLink.addEventListener("click", nextMonth, false);
    } else if (prevLink.attachEvent) {
        prevLink.attachEvent("onclick", prevMonth);
        nextLink.attachEvent("onclick", nextMonth);
    }
